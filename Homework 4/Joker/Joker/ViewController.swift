...
class ViewController: UIViewController
{
    ...
    // segue from the first view to the second view
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        if (segue.identifier == "firstToSecondView")
        {
            let secondViewController = segue.destination as! AddJokeViewController
            
            // increment the numberOfJokes variable from the second view
            // based on the number of jokes in the joke array
            secondViewController.numberOfJokes = jokes.count + 1
        }
    }
